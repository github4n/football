...
function GetDateYMDHMS2(time) {
	var date = new Date(time);
    var year = date.getFullYear();
    var month = formatNum(date.getMonth() + 1);
    var day = formatNum(date.getDate());
    var hh = formatNum(date.getHours());
    var mm = formatNum(date.getMinutes());
    var ss = formatNum(date.getSeconds());
    return year + "-" + month + "-" + day + " "+hh+":"+mm+":"+ss;
}
...
function register(){
	confirm_error1("<lable style=\"color:#000000;font-size:16px;\">提示！</lable><br>参加活动请先注册", function () {
							GoToUrl_T('loginOrRegister.html');
					}, function () {}, "取消", "注册");
}
...
function  DateDiff(sDate1,  sDate2){    //sDate1和sDate2是2006-12-18格式  
	var timeData = GetDateDiffDay(sDate1,sDate2); 
	return timeData.day;
}
    ...
    function ShowCountDown(dom,data){
		...
		for (var i=0;i<data.timArr.length;i++){			
		   ...
		   // var day1 = Math.floor(leftsecond / (60 * 60 * 24));
		    ...
		    if(hour>=0&&minute>=0){
		    	cc[i].innerHTML = text+": "+hour + "小时" + minute + "分";  	
		    }else{
		    	cc[i].innerHTML = "已结束";
