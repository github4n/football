...
<script type="text/javascript">
			...
			$.get(url,function(data){
				if(data.length>0){
					...
					$.each(data,function(i){
						html = "<tr style='height:24px;line-height:24px;' name='subTr"+menuId+"' id='subTr"+this.MENU_ID+"'>";
						html += "<td></td>";
						...
						html += "<td>";
						if(this.MENU_ID==7){
						html +=" <a class='btn btn-mini btn-warning' onclick='openCloseF(\""+this.MENU_ID+"\",this,1)'>展开</a>";
						}
						html +=" <a class='btn btn-mini btn-info' title='编辑' onclick='editmenu(\""+this.MENU_ID+"\")'><i class='icon-edit'></i></a>";
						html +=" <a class='btn btn-mini btn-danger' title='删除' onclick='delmenu(\""+this.MENU_ID+"\",false)'><i class='icon-trash'></i></a>";
						html +=" </td>";
						html += "</tr>";
					...
					if(trIndex%2==0){
						$("tr[name='subTr"+this.MENU_ID+"']").addClass("main_table_even");
					}
				}else{
	...
	function openCloseF(menuId,curObj,trIndex){
		var txt = $(curObj).text();
		if(txt=="展开"){
			$(curObj).text("折叠");
			$("#subTr"+menuId).after("<tr id='ssubTr"+menuId+"'><td colspan='5'>数据载入中</td></tr>");
			if(trIndex%2==0){
				$("#tempTr"+menuId).addClass("main_table_even");
			}
			var url = "<%=basePath%>menu/sub?MENU_ID="+menuId+"&guid="+new Date().getTime();
			$.get(url,function(data){
				if(data.length>0){
					var html = "";
					$.each(data,function(i){
						html = "<tr style='height:24px;line-height:24px;' name='ssubTr"+menuId+"' id='ssubTr"+this.MENU_ID+"'>";
						html += "<td></td>";
						html += "<td><span style='width:100px;display:inline-block;'></span>";
						if(i==data.length-1)
							html += "<img src='static/images/joinbottom.gif' style='vertical-align: middle;'/>";
						else
							html += "<img src='static/images/join.gif' style='vertical-align: middle;'/>";
						...
						html +="</td>";
						html += "</tr>";
						$("#subTr"+menuId).after(html);
					});
					$("#ssubTr"+menuId).remove();
					if(trIndex%2==0){
						$("tr[name='subTr"+menuId+"']").addClass("main_table_even");
					}
				}else{
					$("#subTr"+menuId+" > td").html("没有相关数据");
				}
			},"json");
		}else{
			$("#ssubTr"+menuId).remove();
			$("tr[name='ssubTr"+menuId+"']").remove();
			$(curObj).text("展开");
		}
	}
