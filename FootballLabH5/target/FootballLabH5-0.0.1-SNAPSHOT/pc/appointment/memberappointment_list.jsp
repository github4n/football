...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="memberappointment/pc/list" method="post" name="Form" id="Form">
			<table>
				<tr>
...
<option value="路氹城"<c:if test="${pd.City_NAME == '路氹城' }">selected</c:if> >路氹城</option>
					...
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="MEMBER_NAME" value="${pd.MEMBER_NAME}" placeholder="预约人" />
					...
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="MEMBER_APPOINTMENT_STATUS" data-placeholder="状态" style="vertical-align:top;width: 79px;">
						<option value=""></option>
...
<!-- 						<th>会员ID</th> -->
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<%-- <td>${var.MEMBER_ID}</td> --%>
										<td>${var.City_name}</td>
										<td>${var.MEMBER_VILLAGE_NAME}</td>
										<td>${var.MEMBER_HOU_AREA}</td>
										<td>${var.MEMBER_HOU_BUDGET}</td>
										<td>${var.MEMBER_APPOINTMENT_DATE}</td>
										<td>${var.MEMBER_APPOINTMENT_REMARK}</td>
										<td>${var.MEMBER_APPOINTMENT_TIME}</td>
										<td>
											<c:if test="${var.MEMBER_APPOINTMENT_STATUS ==1}"><span class="label label-important arrowed-in">未受理</span></c:if>
											<c:if test="${var.MEMBER_APPOINTMENT_STATUS ==2}"><span class="label label-success arrowed">已受理</span></c:if>
										</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>memberappointment/pc/goAdd';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>memberappointment/pc/delete?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>memberappointment/pc/goEdit?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>memberappointment/pc/deleteAll?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>memberappointment/pc/excel';
		}
		</script>
