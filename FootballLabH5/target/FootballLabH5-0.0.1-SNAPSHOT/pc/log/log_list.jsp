...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="log/pc/list" method="post" name="Form" id="Form">
			<table>
				<tr>
				
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="LOG_VISITOR" value="${pd.LOG_VISITOR}" placeholder="用户" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="LOG_IP" value="${pd.LOG_IP}" placeholder="IP" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="LOG_DEVICE" value="${pd.LOG_DEVICE}" placeholder="设备" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
...
<option value="路氹城"<c:if test="${pd.City_NAME == '路氹城' }">selected</c:if> >路氹城</option>
					...
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="LOG_SOURCE" id="LOG_SOURCE" data-placeholder="来源" style="vertical-align:top;width: 120px;">
							<option value=""></option>
							<option value="ios" <c:if test="${pd.LOG_SOURCE == 'ios' }">selected</c:if>>ios</option>
							<option value="android" <c:if test="${pd.LOG_SOURCE == 'android' }">selected</c:if>>android</option>
							<option value="微信" <c:if test="${pd.LOG_SOURCE == '微信' }">selected</c:if>>微信</option>
					  	</select>
					</td>
					
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="LOG_URI" id="LOG_URI" data-placeholder="日志类别" style="vertical-align:top;width: 300px;">
							<option value=""></option>
							<option value="/artist/app/designerUser" <c:if test="${pd.LOG_URI == '/artist/app/designerUser' }">selected</c:if>>艺术家-用户浏览筛选设计师的记录</option>
							<option value="/artist/app/designerUser/" <c:if test="${pd.LOG_URI == '/artist/app/designerUser/' }">selected</c:if>>艺术家-用户查看设计师详情的记录</option>
							<option value="/artist/app/designerProject" <c:if test="${pd.LOG_URI == '/artist/app/designerProject' }">selected</c:if>>艺术家-用户浏览筛选案例的记录</option>
							<option value="/artist/app/designerProject/" <c:if test="${pd.LOG_URI == '/artist/app/designerProject/' }">selected</c:if>>艺术家-用户查看案例详情的记录</option>
							<option value="/fashion/app/list" <c:if test="${pd.LOG_URI == '/fashion/app/list' }">selected</c:if>>时尚家-用户浏览频道的记录</option>
							<option value="/fashion/app/fashionLifeDetail" <c:if test="${pd.LOG_URI == '/fashion/app/fashionLifeDetail' }">selected</c:if>>时尚家-用户浏览文章的记录</option>
							<option value="/favorite/app/favoriteAddFashion" <c:if test="${pd.LOG_URI == '/favorite/app/favoriteAddFashion' }">selected</c:if>>时尚家-用户收藏分享文章的记录</option>
							<option value="/thermometer/app/resultdetail" <c:if test="${pd.LOG_URI == '/thermometer/app/resultdetail' }">selected</c:if>>幸福家-用户测量结果的记录</option>
							<option value="/thermometer/app/task" <c:if test="${pd.LOG_URI == '/thermometer/app/task' }">selected</c:if>>幸福家-给用户推送任务的记录</option>
							<option value="/thermometer/app/submitTask" <c:if test="${pd.LOG_URI == '/thermometer/app/submitTask' }">selected</c:if>>幸福家-用户完成任务的记录</option>
							<option value="/optimization/app/index" <c:if test="${pd.LOG_URI == '/optimization/app/index' }">selected</c:if>>楼盘优选-用户搜索的文字记录</option>
							<option value="/optimization/app/detail/" <c:if test="${pd.LOG_URI == '/optimization/app/detail/' }">selected</c:if>>楼盘优选-浏览那些小区的记录</option>
							<option value="/memberappointment/app/saveApply" <c:if test="${pd.LOG_URI == '/memberappointment/app/saveApply' }">selected</c:if>>家装服务-用户预约信息提交的记录</option>
							<option value="/homeDecoration/app/calculator/calc" <c:if test="${pd.LOG_URI == '/homeDecoration/app/calculator/calc' }">selected</c:if>>家装服务-用户计算器提交信息记录</option>
							<option value="/homeDecoration/app/process" <c:if test="${pd.LOG_URI == '/homeDecoration/app/process' }">selected</c:if>>家装服务-用户浏览了装修流程的记录</option>
							<option value="/homeDecoration/app/shopDetail/" <c:if test="${pd.LOG_URI == '/homeDecoration/app/shopDetail/' }">selected</c:if>>家装服务-用户浏览门店的记录</option>
							<option value="/campaign/app/detail/" <c:if test="${pd.LOG_URI == '/campaign/app/detail/' }">selected</c:if>>营销活动-用户浏览活动的记录</option>
							<option value="/campaignapply/app/campaignApplyAdd" <c:if test="${pd.LOG_URI == '/campaignapply/app/campaignApplyAdd' }">selected</c:if>>营销活动-用户活动报名信息的记录</option>
							<option value="/qrcode/app/addresult" <c:if test="${pd.LOG_URI == '/qrcode/app/addresult' }">selected</c:if>>二维码-用户扫描二维码URL的记录</option>
							<option value="/home/app/login" <c:if test="${pd.LOG_URI == '/home/app/login' }">selected</c:if>>个人中心-用户登陆的记录</option>
							<option value="/app/user/memregister" <c:if test="${pd.LOG_URI == '/app/user/memregister' }">selected</c:if>>个人中心-用户注册的记录</option>
							<option value="/app/user/info/save" <c:if test="${pd.LOG_URI == '/app/user/info/save' }">selected</c:if>>个人中心-用户信息修改的记录</option>
							<option value="/favorite/app/favoriteAdd" <c:if test="${pd.LOG_URI == '/favorite/app/favoriteAdd' }">selected</c:if>>个人中心-用户收藏记录</option>
							
					  	</select>
					</td>
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="LOG_MEMBER_TYPE" data-placeholder="游客/会员" style="vertical-align:top;width: 79px;">
						<option value=""></option>
						<option value="0" <c:if test="${pd.LOG_MEMBER_TYPE == '0' }">selected</c:if> >游客</option>
						<option value="1" <c:if test="${pd.LOG_MEMBER_TYPE == '1' }">selected</c:if> >会员</option>
						</select>
					</td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>${var.LOG_VISITOR}</td>
										<td>${var.LOG_DEVICE}</td>
										<td>${var.LOG_IP}</td>
										<td>${var.LOG_CITY}</td>
										<td title="${fn:replace(var.LOG_URI, '"', '')}">${fn:substring(var.LOG_URI,0,38)}</td>
										<%-- <td title="${fn:replace(var.LOG_PARM, '"', '')}">${fn:substring(var.LOG_PARM,0,12)}</td>
										<td title="${fn:replace(var.LOG_RESULT, '"', '')}">${fn:substring(var.LOG_RESULT,0,12)}</td> --%>
										<td>${var.LOG_STARTTIME}</td>
										<td>${var.LOG_ENDTIME}</td>
										<%-- <td>${var.LOG_LONGITUDE}</td>
										<td>${var.LOG_LATITUDE }</td> --%>
										<td>${var.LOG_SOURCE}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>log/delete?LOG_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>log/goEdit?LOG_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>log/pc/deleteAll?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>log/excel';
		}
		</script>
