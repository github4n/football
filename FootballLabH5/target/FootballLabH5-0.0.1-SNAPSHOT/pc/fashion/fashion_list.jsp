...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="fashion/pc/list?isM1=${pdm.isM1 }&isM2=${pdm.isM2 }" method="post" name="userForm" id="userForm">
			<table border='0'>
				<tr>
					...
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="recommand" data-placeholder="是否推荐" style="vertical-align:top;width: 79px;">
						<option value=""></option>
					...
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="type" data-placeholder="类型" style="vertical-align:top;width: 79px;">
						<option value=""></option>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							...
							<tr>
								...
								<td title="${var.title}">${fn:substring(var.title ,0,30)} ...</td>
								<td>
									<c:if test="${var.type == '1' }">家生活</c:if>
									<c:if test="${var.type == '2' }">名人典故</c:if>
									<c:if test="${var.type == '3' }">360全景</c:if>
								</td>
								<td>${var.publisher }</td>
								<td>${var.starttime }</td>
								<td>${var.endtime }</td>
								<td>
									<c:if test="${var.recommand == '0' }"><span class="label label-success arrowed-in">未推荐</span></c:if>
		...
		<table style="width:100%;">
			<tr>
				...
				<c:if test="${QX.edit == 1 }">				
					<td style="vertical-align:top;"><a class="btn btn-small btn-success" onclick="makeAll('确定要发布选中的数据吗?');" title="批量发布" >批量发布</a></td>
				</c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>fashion/pc/goAdd';
			 diag.Width = 800;
			 diag.Height = 650;
		...
		function edit(id){
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>fashion/pc/goEdit?id='+id;
			 diag.Width = 800;
			 diag.Height = 650;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(id){
			bootbox.confirm("确定要删除该记录?", function(result) {
				if(result) {
					var url = "<%=basePath%>fashion/pc/delete?id="+id+"&tm="+new Date().getTime();
					...
					if(result) {
						...
						if(str==''){
							...
							if(msg == '确定要发布选中的数据吗?'){
								$.ajax({
									type: "POST",
									url: '<%=basePath%>fashion/pc/pubAll?tm='+new Date().getTime(),
							    	data: {DATA_IDS:str},
									dataType:'json',
