...
/* 		if($("#content").val()==""){
	...
	function delP(tpurl,id){
		 if(confirm("确定要删除图片？")){
			var url = "<%=basePath%>link/deltp?tpurl="+tpurl+"&id="+id+"&guid="+new Date().getTime();
...
<body>
	<form action="link/${msg }" name="Form" id="Form" method="post" enctype="multipart/form-data">
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
			<tr>
					...
					<td><select name="expert_id" id="expert_id">
							<c:forEach items="${experts}" var="expert">
								<option value="${expert.EXPERTS_ID}" <c:if test="${expert.EXPERTS_ID == pd.fk_experts_id }">selected</c:if> >${expert.EXPERTS_NAME}</option>
							</c:forEach>
					</select></td>
				</tr>
			...
			<tr>
				<td>标题:</td>
				<td><input type="text"style="width:95%;" name="stiename" id="stiename" value="${pd.stiename }" placeholder="这里输入标题" title="标题"/></td>
			</tr>
 			<%--
			<tr>
				<td>内容:</td>
			...
			<tr>
				...
				<td>
					...
					<c:if test="${pd != null && pd.stieurl != '' && pd.stieurl != null }">
						<a href="http://www.01588.com/pic/${pd.stieurl}" target="_blank"><img src="http://www.01588.com/pic/${pd.stieurl}" width="210"/></a>
