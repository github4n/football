...
<script type="text/javascript">
	...
	function delP1(image1,id){
		 if(confirm("确定要删除图片？")){
			var url = "<%=basePath%>campaign/pc/deltp1?image1="+image1+"&id="+id+"&guid="+new Date().getTime();
	...
	function delP2(image2,id){
		 if(confirm("确定要删除图片？")){
			var url = "<%=basePath%>campaign/pc/deltp2?image2="+image2+"&id="+id+"&guid="+new Date().getTime();
			$.get(url,function(data){
				if(data=="success"){
					alert("删除成功!");
					document.location.reload();
	...
	function delP3(image3,id){
		 if(confirm("确定要删除图片？")){
			var url = "<%=basePath%>campaign/pc/deltp3?image3="+image3+"&id="+id+"&guid="+new Date().getTime();
			$.get(url,function(data){
				if(data=="success"){
					alert("删除成功!");
					document.location.reload();
...
<body>
	<form action="campaign/pc/${msg }" name="Form" id="Form" method="post" enctype="multipart/form-data">
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<tr>
				...
				<td>
					...
					<c:if test="${pd != null && pd.image1 != '' && pd.image1 != null }">
						<a href="uploadFiles/campaign/${pd.image1}" target="_blank"><img src="uploadFiles/campaign/${pd.image1}" width="210"/></a>
			...
			<tr>
				...
				<td>
					...
					<c:if test="${pd != null && pd.image2 != '' && pd.image2 != null }">
						<a href="uploadFiles/campaign/${pd.image2}" target="_blank"><img src="uploadFiles/campaign/${pd.image2}" width="210"/></a>
			...
			<tr>
				...
				<td>
					...
					<c:if test="${pd != null && pd.image3 != '' && pd.image3 != null }">
						<a href="uploadFiles/campaign/${pd.image3}" target="_blank"><img src="uploadFiles/campaign/${pd.image3}" width="210"/></a>
			...
			<tr>
				...
				<td>
					...
					<input type="text" name="telephone" id="telephone" value="${pd.telephone }" placeholder="这里输入咨询电话" title="咨询电话" />
					<input type="text" name="address" id="address" value="${pd.address }" placeholder="这里输入活动地址" title="活动地址" />
				</td>
			</tr>
			<tr>
				<td>选择:</td>
