						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<td><fmt:formatDate value="${var.transaction_create_time}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
										<td>${var.transaction_number}</td>
										<c:choose>
										...
										<td>
											<c:if test="${var.transaction_status == '1' }">成功</c:if>
											<c:if test="${var.transaction_status == '0' }">失败</c:if>
										</td>
