  ...
  <div class="row-fluid">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id,PATH){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>pictures/delete?PICTURES_ID="+Id+"&PATH="+PATH+"&tm="+new Date().getTime();
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>pictures/deleteAll?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>pictures/excel';
		}
		</script>
		<style type="text/css">
