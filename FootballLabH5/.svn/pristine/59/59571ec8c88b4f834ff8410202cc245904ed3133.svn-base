...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="link/list?isM1=${pdm.isM1 }&isM2=${pdm.isM2 }" method="post" name="userForm" id="userForm">
			<table border='0'>
				<tr>
					<td>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							...
							<tr>
								...
								<td>${fn:substring(var.stiename ,0,20)}</td>
								<td>${var.addtime }</td>
 								<%--
								<td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function edit(id){
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>link/goEdit?id='+id;
			 diag.Width = 800;
			 diag.Height = 650;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
