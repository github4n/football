        ...
        function (res) {
            ...
            if (res.err_msg == "get_brand_wcpay_request:ok") {
            	 if(type==1){
            		 ...
            		 var res = GetData(fl_url.followPayPage,param);
            		 res = JSON.parse(res);
            		 confirm_error1("跟单成功"
            				 ,function(){//查看详情
        					 location.href="expertPlan_detail.html?experts_code="+getQueryString("experts_code")+"&showType=1&followType=1"+"&gameDate="+d_t_ydm+"&companyId=" + res.companyId+"&serviceType=" + res.serviceType+"&type=" + res.type +"&serviceId=" + param.service_id + "&service_code=" + res.service_code ;
            		 },function(){//继续跟单
